...
void main()
{
    ...
    vec2 advected_velocity =
        s0 * (t0 * imageLoad(in_img, pos0).xy + t1 * imageLoad(in_img, ivec2(pos0.x, pos1.y)).xy) +
        s1 * (t0 * imageLoad(in_img, ivec2(pos1.x, pos0.y)).xy + t1 * imageLoad(in_img, pos1).xy);


    imageStore(out_img, tex_coords, vec4(advected_velocity, data.z, 0.0));
}
